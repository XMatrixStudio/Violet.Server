...
exports.getList = async ctx => {
  ctx.body = await authServer.getList(ctx.getUserId(ctx))
}
...
exports.get = async ctx => {
  ...
  let auth = await authServer.get(ctx.getUserId(ctx), body.clientId)
  ctx.body = {
    auth: auth
  }
}
...
exports.delete = async ctx => {
  ...
  verify({ data: body.clientId, type: 'string', maxLength: 24, minLength: 24, message: 'invalid_clientId' })
  await authServer.delete(ctx.getUserId(ctx), body.clientId)
  ctx.status = 200
}
